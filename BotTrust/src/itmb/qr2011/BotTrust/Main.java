...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        Scanner scanner = new Scanner(new File("A-Test.in"));
        BufferedWriter bf = new BufferedWriter( new FileWriter("A-Test.out"));
        ...
        for (int i=0; i< T; i++){
            ...
            for (int j =0 ; j < N ; j++){
                ...
                if (tmp.equals("O")){
                    ...
                    oTime = max(oTime + abs(next - oPosition), bTime)  + 1;
                    oPosition = next;
                } else {
                    int next = scanner.nextInt();
                    bTime = max(oTime,bTime + abs(next - bPosition))  + 1;
                    bPosition = next;
                }
            }
            bf.write("Case #" + (i+1) + ": " + max(oTime,bTime) + "\n");
            System.out.println("Case #" + (i+1) + ": " + max(oTime,bTime));
        }
        bf.flush();
    }
}
